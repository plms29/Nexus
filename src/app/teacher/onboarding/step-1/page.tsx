...
export default function Step1Page() {
  ...
  const handleNext = () => {
    if (!state.teacherName.trim() || !state.schoolName.trim()) {
      setError(tr("Vui lòng điền đầy đủ thông tin"));
      return;
    }
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="bg-white border border-slate-200 rounded-2xl p-8 shadow-lg relative overflow-hidden">
        ...
        <div className="mb-8">
          <p className="text-blue-600 font-bold text-xs tracking-widest uppercase mb-2">{tr("BƯỚC 1 / 3")}</p>
          <h2 className="text-2xl font-bold text-slate-900 mb-2">{tr("Thông tin hồ sơ giáo viên")}</h2>
          <p className="text-slate-600 text-sm">{tr("Nhập tên của thầy/cô, trường THPT và Tỉnh/Thành phố đang công tác để cá nhân hóa hệ thống.")}</p>
        </div>

        <div className="space-y-6">
          <div className="space-y-2">
            <label className="text-xs font-semibold text-slate-700 uppercase tracking-wider">{tr("Họ và Tên Giáo Viên")}</label>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="space-y-2">
              <label className="text-xs font-semibold text-slate-700 uppercase tracking-wider">{tr("Trường THPT")}</label>
              <div className="relative">
                <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none text-slate-500">
            ...
            <div className="space-y-2">
              <label className="text-xs font-semibold text-slate-700 uppercase tracking-wider">{tr("Tỉnh / Thành Phố")}</label>
              <div className="relative">
                <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none text-slate-500">
